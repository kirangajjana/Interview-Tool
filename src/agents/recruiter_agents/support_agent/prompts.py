...
1. **Genuine Technical Issues (Set `is_genuine_technical_issue` to True, `auto_resolve_eligible` to True)**:
...
2. **Manual Review Required (Set `is_genuine_technical_issue` to False, `auto_resolve_eligible` to False)**:
   ...
   - Requests for retakes due to poor performance (e.g. "I did bad on my MCQ, I want to try again to get a better score").
