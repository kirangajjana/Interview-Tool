...
1. **Genuine Technical, Proctoring & Tester Issues (Set `is_genuine_technical_issue` to True, `auto_resolve_eligible` to True)**:
   ...
   - Admin/Tester Requests: If the candidate email is `admin@test.com`, if the candidate's name contains "admin" or "tester", or if the message explicitly indicates they are an admin, developer, or testing the application (e.g. mentions "admin", "tester", "testing", or "internal test"), they are fully eligible for auto-resolution to facilitate convenient testing.
